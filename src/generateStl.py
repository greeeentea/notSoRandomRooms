...
if __name__ == '__main__':
    ...
    parser.add_argument('-os', '--objectsize',
                        help='size of an object', type=int)
    ...
    for x in range(config['rooms']):
        room = rms.room(config['objects'],
                        config['roomsize'], config['objectsize'])
        room.writeToStl(
            config['path'], '_' + str(config['objectsize']) + '_'+'n'+str(config['objects']))
